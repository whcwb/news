...
<script>
    ...
    export default {
        ...
        methods: {
            getPermissionTree(){
                this.$http.get(this.apis.FUNCTION.GET_ORG_PERMISSION_TREE,{timers:new Date().getTime()}).then((res) =>{
                    if(res.code===200){
			...
			getRolePermission(){
                this.$http.get(this.apis.FUNCTION.GET_ROLE_FUNCTIONS+"?jsdm="+this.addmess.jsId,{timers:new Date().getTime()}).then((res) =>{
                    if(res.code===200){
