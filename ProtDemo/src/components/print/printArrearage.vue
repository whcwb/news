  ...
  export default {
    ...
    methods: {
      ...
      getNum(){
        ...
        setTimeout(()=>{
          v.AF.getPrintNum('arrear',this.studentIDS,num => {
            v.num = num
          })
      ...
      getMess(arr,callback) {
        ...
        arr.forEach((item, index) => {
          ...
          this.money += item.chargeRecord.chargeFee
          let jgmcArr = ['']
      ...
      BDNum(){
        ...
        this.$http.post('/api/chargemanagement/savePjbh',{ids:ids,num:'arrear',pjbh:this.num}).then(res=>{
          if(!res.code == 200){
            return
          }else {
            v.$parent.getPagerList1()
          }
        }).catch(err=>{
          return
        })
