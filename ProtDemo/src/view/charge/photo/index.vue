<template>
  <div class="box_col" style="position: relative">
    <pager-tit title="图片管理"></pager-tit>
    <Row>
      <!--      <div>-->
  ...
  export default {
        ...
        tabTitS: [
          ...
          },
          {
            ...
            filterMethod(value, row) {
              return row.type == Number(value)-1
            }
          },
          {
            title: '操作',
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        content: "<p>确认删除该图片？</p>",
                        onOk: () => {
                          this.$http.post(this.apis.IMG.DEL+p.row.id)
                            .then(data => {
                              if (data.code == '200') {
    ...
    methods: {
      ...
      getData() {
        ...
        this.$http.post(this.apis.IMG.GETIMG,this.param).then(res=>{
            if(res.code=='200'){
