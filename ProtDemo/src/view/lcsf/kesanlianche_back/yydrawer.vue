    ...
    >
      ...
      <Form :model="formData">
        <Row :gutter="32">
          ...
          <Col span="12" v-if="formData.jlLx == '10'">
            <FormItem label="所属驾校" label-position="top">
              <Select filterable clearable v-model="jlJx" placeholder="请选择驾校" @on-change="getjxjllist" @on-clear="onclear">
        ...
        <component v-if="formData.jlLx == '10'" :is="compName" :jxmc="jlJx"
                   @SaveOk="addjlSaveOk"
                   @colse="compName=''"
                   @jxSeljxSel="(val)=>{jlJx=val,getwxJLList(val)}"></component>
        <Divider/>
        <Row v-if="formData.jlLx == '10'" style="background-color: #f2f2f2">
          <Card>
            <p slot="title">学员信息</p>
            <p>
